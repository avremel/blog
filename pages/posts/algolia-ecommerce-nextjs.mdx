...
This is _not_ intended as a comprehensive guide (the docs are superb) but rather notes on how to avoid common footguns. The second half discusses the frontend clients.
...
**In my experience**:
- Afer a page refresh, don't add or remove hooks (the same goes for `getServerState`). For example, if you want to display a series filter only if a brand selection was made, render both filters, but hide the series filter until it is needed.

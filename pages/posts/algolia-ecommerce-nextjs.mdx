...
Many ecommerce sites will have a single product card for multiple variantions of that item like color or size. To configure that functionality, you will [need to](https://www.algolia.com/doc/guides/managing-results/refine-results/grouping/how-to/item-variations/):
- Create a record for every variant.
- Turn on Distinct (`distinct`) and choose an attibute to group variants (`attributeForDistinct`).
- If you want to display meaninful information for each variant (like price, link, images), each variant will need to duplicate the data for _every_ one of it's sibling variants.

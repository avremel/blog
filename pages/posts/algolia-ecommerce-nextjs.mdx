...
4. To control which variant shows up as the primary record, add a strict sort to Ranking and Sorting (SORT BY `is_default_varaint`).
...
:point_right: Filtering won't remove irrelevant variants from a recored. For example, if you filter by blue, yellow will still show up. You'll need to remove yellow with application code.
...
:point_right: Merchandising rules (and other settings) get copied over to new sort indices and can potentially interfere with sorting. Make sure to remove them as needed.
...
:round_pushpin: Visual and manual rules cannot coexist for the same triggers, so you will have to choose one or the other.
...
Another consequence is that in order to get server-side-rendering (SSR) working for NextJS, (somewhat confusingly) you need to:
...
:point_right: Since you are running your app server side solely to get the Algolia state, you only care about rendering Algolia hooks and can avoid any other expensive side effects ([docs](https://www.algolia.com/doc/api-reference/widgets/server-state/react/#widget-param-children)).
...
**Some hard-learned lessons from wrestling with React InstantSearch**:
- Afer a page refresh, don't add or remove hooks (the same goes for `getServerState`). For example, if you want to display a filter for series only if a brand selection was made, render both filters, but hide the series filter until it is needed.
...
- Sometimes you want to calculate the UI by taking a look at the data returned by all filters _at the same time_[^1]. The problem is that merging data from a bunch of hooks is [_painful_](https://github.com/algolia/instantsearch/discussions/5353#discussioncomment-4330951) and brittle.

[^1]: For example, if you have a filter group called Dimensions which contains height, width and depth filters. If there are no values for any of the filters, you would probably want to hide the entire filter group. A possible workaround is doing something like `.filter-group::has(.facets:empty) { display: none }`.

{/* ## Usage

- debounce autocomplete
- verify network tab that clicking onf filteers doesn't dupe request

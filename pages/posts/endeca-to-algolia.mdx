...
[Faceted search](https://en.wikipedia.org/wiki/Faceted_search) has been powering e-commerce since the dawn of the internet. Endeca has been our go-to solution for close to two decades but the time is ripe for change. As with any migration, the replacement - Algolia - is gooder is some ways and badder in others.

|Endeca  |Algolia |
|---|---|
|Self-host data ingestion infra (Endeca Forge)|Data ingestion is as simple as POSTing to an API|
...
|Write all UI code yourself|Provides UI libraries for all popular frameworks. You can either use out-of-the-box components and customize the CSS or use [headless components](https://medium.com/@nirbenyair/headless-components-in-react-and-why-i-stopped-using-ui-libraries-a8208197c268) for added flexibility.|

#### Algolia is gooder

- [Rules](https://www.algolia.com/doc/guides/managing-results/rules/rules-overview/) (or [Precedence Rules](https://docs.oracle.com/cd/E70265_01/config.11-3/Endecaconfig/html/gscg_precedence_rules.xmlconcept_07D2A3108D044CD38274C1BCEBBA9C4C.html) in Endeca) is the ability to provide a unique user experience for a particular page. For example, if the user is searching for "range hoods", bury `ranges` and boost `range hoods`. Algolia has more rule based customizations than Endeca like boost/bury categories, hide items, pin items, return custom data, etc.
- [Insights](https://www.algolia.com/doc/guides/building-search-ui/going-further/send-insights-events/js/) track user interactions (items viewed, filters clicked, items clicked, conversions, etc).
...
Quotes from [here](https://www.algolia.com/doc/rest-api/search/). The use of their frontend libraries is clearly the recommended path.
...
- Until very recently, no native support for [Next/Link](https://nextjs.org/docs/api-reference/next/link), see [this long running issue](https://github.com/algolia/instantsearch/issues/5241). An official [package](https://www.npmjs.com/package/react-instantsearch-hooks-router-nextjs) is now available, but in my experience isn't stable.
- Client side filtering which trigger a rule seem to cause the filter widgets to unmount, which in turn clears their selection. In these cases, I force a page reload.
  - _Possible workaround: Force a page refresh on filters which trigger a rule change._
...
[Why is it so hard to buy things that work well](https://danluu.com/nothing-works/) is a thoughtful post on the buy vs. build debate.

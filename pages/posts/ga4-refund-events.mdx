...
export const meta = {
  ...
  description: 'How to send offline refund events with Google Analytics 4 and debugging with BigQuery',
  date: 'Oct 2023',
...
The sum of these events - purchases minus refunds - are reflected in `Reports > Total Revenue`.
...
You can retreive the `client_id` by storing the `_ga` cookie in your database at checkout time, or by querying BigQuery:
```sql
SELECT
...
I haven't figured out how to inspect individual refund events in the GA4 UI using `Explore` as the UI is shockingly bad. Instead, I'm using the BigQuery integration to verify that events are coming in correctly. These are queries I've been using:

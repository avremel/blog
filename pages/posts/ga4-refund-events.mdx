...
> :point_right: A timestamp can be done using the `timestamp_micros`. However, the protocol only supports timestamps less than 72 hours ago. _If_ you have to wait 48 hours to send a refund event, the refund timestamp might be stale by the time it reaches the API, causing the event to get rejected.

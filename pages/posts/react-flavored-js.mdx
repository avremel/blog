...
<Aside pTags={
  [
    `The goal is to demonstrate how to create stateful + reusable components with JS/jQuery. I'm picking on React because that's what I'm most comfortable with.`,
  ]} />
...
- Whereas React effectively tracks `prop` changes and triggers rerenders, we must explicitly notify the component of `prop` changes (with `setValues`).
...
- Traditionally, you render HTML upfront and mutate it as needed. React allows you to declare your UI, and manages DOM updates for you on `prop/state` change. We _could_ emulate this approach by tearing down the DOM on `prop/state` change, [but why would we?](https://svelte.dev/blog/virtual-dom-is-pure-overhead)

## In Conclusion

If you have tolerance for discomfort, jQuery + modern JS can be a mediocre subsitute for modern frameworks. You will lose the awsome DX (Developer Experience) but you only have yourself to blame 😉.
